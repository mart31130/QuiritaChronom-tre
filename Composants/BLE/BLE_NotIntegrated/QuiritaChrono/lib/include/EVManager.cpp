...
void NewObjectCounter(){newObject++;}
...
void SendAll(){
	EV event; int32_t param;
  Debug << "fsmCount=" << fsmCount << '\n';
  Debug << "Objets créés= " << newObject << '\n';
	while( Pull(&event,&param) ) for( auto i=0; i<fsmCount; i++ ) if( fsmlist[i]->Event(event,param) ) break; // exit for si un listener a consommé l'évènement
