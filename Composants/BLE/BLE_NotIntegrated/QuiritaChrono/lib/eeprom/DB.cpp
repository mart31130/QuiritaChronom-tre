...
#define TUPLETSIZELOG (1+EEPAGESIZELOG)
...
void _db_WrTuplet(const char* keyName, uint8_t* value, uint8_t valueSize,  uint16_t eeBaseAddr){
	...
	db.eeprom.EepromWrite(eeBaseAddr,EEPAGESIZE,buffer);
	//------ enregistrement du contenu de la variable
	db.eeprom.EepromWrite(eeBaseAddr+EEPAGESIZE,valueSize,value);
	
}//_db_WrTuplet()

//initialisation de la base de donnée NOSQL {clef,valeur}
uint16_t CDb::db_init(int dummyArg){
	...
	eeprom.EepromRead(EEPAGESIZE, sizeof(dbAT_t), (uint8_t*)&dbAllocationTable);
...
uint16_t CDb::db_info(uint32_t* version, uint16_t* maxKey, uint16_t* keyCount, uint32_t* mpuuid){
...
uint16_t CDb::db_set(const char* key, uint8_t* value, size_t valueSize){
	...
	if(index==0){ // un nouveau tuplet doit être créé
		...
		eeprom.EepromWrite(EEPAGESIZE,sizeof(dbAllocationTable),(uint8_t*)&dbAllocationTable);
	}else{ // tuplet existant => mise à jour de la valeur 
		eeprom.EepromWrite((index<<TUPLETSIZELOG)+EEPAGESIZE,valueSize,value);
	}
	return index;
}//db_set()
...
uint16_t CDb::db_find(const char* key){
    ...
    if((candindex>0)&&(strcmp(key,candidat)==0)) return candindex;
  	...
  	for(candindex=1; candindex<dbAllocationTable.KeyCount; candindex++){
    		...
    		eeprom.EepromRead(eeAddr,EEPAGESIZE,(uint8_t*)candidat);
...
uint16_t CDb::db_get(const char* key, uint8_t* value, size_t size){
	uint16_t index=db_find(key);
	return (index)?db_get(index,value,size):index;
}//db_get

uint16_t CDb::db_get(uint16_t index, uint8_t* value, size_t size){
	uint16_t eeAddr=(index<<TUPLETSIZELOG)+EEPAGESIZE;
	eeprom.EepromRead(eeAddr,size,value);
	return index;
}//db_get()

uint16_t CDb::db_getTuplet(uint16_t index, char* key, uint8_t* value, size_t* valueSize){
  uint16_t eeAddr=(index<<TUPLETSIZELOG);
  // renvoyer la clef
  eeprom.EepromRead(eeAddr,EEPAGESIZE,(uint8_t*)key);
  uint8_t dataSize=key[EEPAGESIZE-1];
  *valueSize=dataSize;
  // renvoyer la valeur
  eeAddr+=EEPAGESIZE;
  eeprom.EepromRead(eeAddr,*valueSize,value);
  return index;
}//db_getTuplet()
...
uint16_t CDb::db_del(const char* key){
    ...
    uint8_t  tmp[EEPAGESIZE];
		...
		eeprom.EepromRead( eeSrc,EEPAGESIZE,tmp);
		eeprom.EepromWrite(eeDst,EEPAGESIZE,tmp);
		// copy value[KeyCount] to value[index]
		eeprom.EepromRead( eeSrc+EEPAGESIZE,EEPAGESIZE,tmp);
		eeprom.EepromWrite(eeDst+EEPAGESIZE,EEPAGESIZE,tmp);
