...
inline void _updateAllocationTable() {
...
__STATIC_INLINE uint32_t stm32F4uuid() {
  return STM32_UUID[0] ^ STM32_UUID[1] ^ STM32_UUID[2];
}  // spécifique STM32F4
...
uint16_t db_del(uint16_t index) {
  ...
  if (index < head) {                                     // on va écraser le tuplet index avec le tuplet du haut
    uint8_t valtmp[EEPAGESIZE];
    char keytmp[EEPAGESIZE];
    size_t valsize;
...
// affiche la variable enregistrée à l'index en paramètre
void AfficheDB(uint16_t index) {
  char clef[MAXSIZEOF] = "";
  uint8_t valeur[MAXSIZEOF] = { 0 };
  char ascii[MAXSIZEOF + 1] = "";
  size_t valueSize = 0;
  if (index >= dbAllocationTable.KeyCount) {
    shell << "Data don't exist\n";
    return;
  }
  db_getTuplet(index, clef, valeur, &valueSize);
  //---- affiche le nom et la taille
  if (index < 9) shell << "0";
  shell << index << " " << clef << "[" << valueSize << "] ";
  //---- aligne les sorties
  for (size_t s = strlen(clef) + (valueSize < 9 ? 1 : 2); s < 20; s++) shell << " ";
  //---- affiche le contenu en hexa
  for (size_t j = 0; j < valueSize; j++) {
    char b, c = valeur[j];
    if (j % 4 == 0) shell << " ";
    b = c >> 4;
    shell << (char)((b < 10) ? ('0' + b) : ('A' + b - 10));
    b = c & 0x0F;
    shell << (char)((b < 10) ? ('0' + b) : ('A' + b - 10));
  ...
  ascii[valueSize] = 0;
  shell << " " << ascii;
  //---- affiche la valeur en décimal lorsque la taille est exactement de 4 octets
  if (valueSize == 4) {
    uint32_t u = *(uint32_t*)valeur;
    shell << " " << u;
  }
  shell << "\n";
}  //AfficheDB(uint16_t index)
...
void AfficheDB() {
  ...
  for (uint32_t index = 0; index < keyCount; index++) AfficheDB(index);
  shell << "\n";
}  //AfficheDB
